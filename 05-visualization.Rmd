...
Perhaps the most frequent mistake `ggplot` newcomers make is conflating aesthetic *mapping* with aesthetic *setting*. 

Instead of coloring points according to "type" of car, you may wish to set all points to a single color, say, blue. So you devise the following `ggplot` code. 
...
As we can see, this code colors all points *red*, not blue! Before reading on, can you spot the mistake we made? 

When we wish to *set* an aesthetic to some global value (e.g., "red", "triangle", an alpha of "0.5"), it __must not be included as part of the mapping specification in the `aes` call__. 
...
```{r dpi=80, fig.align="center", message = FALSE}
ggplot(happy_students) + 
  geom_bar(aes(x = as_factor(happiness), y = number), stat = "identity") +
  xlab("Happy score") + 
  ylab("Count")
```
...
```{r dpi=80, fig.align="center", message = FALSE}
ggplot(happy) +
  geom_bar(aes(x = happy, fill = as_factor(love)), position = "fill")  +
  labs(fill = "love")
```
...
```{r dpi=80, fig.align="center", message = FALSE}
ggplot(happy) +
  geom_bar(aes(x = happy, fill = as_factor(love)), position = "dodge")+
  labs(fill = "love")
```
...
```{r dpi=80, fig.align="center", message = FALSE}
ggplot(happy) +
  geom_bar(aes(x = as_factor(happy), fill = as_factor(love)), position = "dodge") +
  xlab("Happy Score") +
  labs(fill = "love") +
...
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(aes(y = after_stat(density)), fill = "darkcyan", color = "grey", 
...
We can generate a histogram in density scale for each subset. Note that both histograms apply the same bin choices given in `bin_choices`. 
...
g1 <- ggplot(first_subset, aes(x = hwy)) +
  geom_histogram(aes(y = after_stat(density)), 
                 ...
                 color = "grey", 
                 breaks = bin_choices) +
  labs(title = "minivan, midsize, and suv")
...
g2 <- ggplot(second_subset, aes(x = hwy)) +
  geom_histogram(aes(y = after_stat(density)), 
                 ...
                 color = "grey", 
                 breaks = bin_choices) +
  labs(title = "compact, subcompact, and 2seater")
...
## Vignette: Exploring the US Census 
