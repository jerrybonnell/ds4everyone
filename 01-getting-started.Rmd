...
We call this one a *stacked bar chart*, which we will also see later. Every plot tells a different story. This one signals how the *proportion* of telephones changes across each region. For instance, the stacked bar chart makes it easier to see how the regions Asia and Africa gradually increase its share of telephone usage when compared to the scatter plot.       
...
There are two ways to work with R. The first option is Posit Cloud, a lightweight and cloud-based interface to RStudio. If you use Posit Cloud, you can code right from your favorite browser! However, because Posit Cloud is cloud-based, the code you write is executed on someone else's machine and, therefore, uses up their computational resources. For this reason, Posit Cloud offers a "free tier" that gives access to limited resources (1GB memory and 1 CPU per project) with 25 project hours/month. This should be sufficient for the examples and exercises covered in this text, but you may run up against the project hour quota. You may wish to upgrade to a paid tier that gives more project hours and computational power. We recommend Posit Cloud to students with older machines and students who prefer a zero-installation setup for getting started. Posit Cloud is available here:

* Posit Cloud: https://posit.cloud/ 

The second option is to install R and RStudio Desktop directly to your system. First, you will need to install R and RStudio Desktop from the web. They can be found at the following links. 
...
If you are working on Posit Cloud, navigate to your workspace and create a new RStudio project by clicking "New Project" and then "New RStudio Project". This should bring you into a RStudio environment, as shown in the next section.  

If you installed RStudio Desktop locally, launch the RStudio application. There may be some "getting started" help guides that you are presented with, which you can ignore for now. Your next step will be to create a __R project__ by going to: `File > New Project...`. 
