...
In the previous chapter we learned that using randomness we can make selections by chance.
In this chapter, we will use sampling more aggressively and learn to exploit the power of sampling.
Specifically, we will learn that (a) by drawing samples we are able to observe an unknown distribution and (b) by conducting experiments by sampling we can assess how unlikely or likely it is a phenomenon at hand.
In both cases, the number of samples we draw plays a big role.


__Prerequisites__

Before starting, let use load `tidyverse` as usual.

```{r, message = FALSE, warning = FALSE}
library(tidyverse)
```

Many data sets data scientists handle, including those data sets you have access as part of RStudio, are tables.
We will familiarize ourselves with the use of sampling with table data.
In this chapter specifically, we will use the `mpg` data set that comes with R.
We will also use the `flight` data that we can download from the Bureau of Transportation Statistics.
...
Recall that on a table-like data set, we often anticipate the columns of the table (that is, the vertical slices) to represent the properties we can obtain from an individual object.
A collection of the properties representing an individual is a *record* or a *data object*.
The rows of a data set are the data records.
The row X and column Y of the data set is the property Y of the data object X.

By "sampling" we mean to select rows from the data.
If we want to select distinct rows of the data, there is a convenient function `slice` for the action, which you may recall.
The first argument of the function call specifies the source for sampling, and the other specifies the rows we draw with a list representation.
...
As you see, the `slice` function call here generates a new data set with Rows 3, 25, and 100 of `mpg`.
The function `mpg` does not care if the row numbers appearing in the argument are all different or if the row numbers appear in non-decreasing order.
By repeating a pair `3` and `25` four times, you obtain a subset with four repeats of the consecutive appearance of Audi A4 and Chevrolet Corvette.
...
When you know which rows should appear a sample data set, you state them in your process to create the data set.
We call the sampling process with a selection list of row *deterministic sample*.
The determinism refers to the non-existence of chances (unless the list of rows comes from an external source and the process of generating the list uses randomness).

An alternative to directly specifying the row numbers is specifying a condition on an attribute for a record to be in the sample data set.
We have seen this before.
The name of the function is `filter`.
...
There is room for ambiguity in filtering.
The condition on the attribute of a record either puts in the subset or it does not. Exactly one of the two occurs.
Like `slice`, `filter` selects the records deterministically.
If we calculate by hand check for each row, whether it is part of the subset.
The command `filter` produces exactly the same result as we do by hand.


__Probabilistic Sampling__

While the antonym of "deterministic" is "non-deterministic" and the concept plays an important role in computer science, data science does not use the word.
The word only states that the process is not deterministic.
It does not state how to draw non-determinism.
Instead, data science uses a more specific form of non-determinism "randomness".
We call the process of sampling using randomness *probabilistic sampling* or *random sampling*.
The objects from which we can draw samples are the *population*.
There are some types of populations.
The determining factors of the types are their quantity and the we generate samples.
Here are some (non-comprehensive) examples of populations and how we want to sample from the data.
...
* *Continuous Population Requiring Discretization* We want to study the quality of air based on how ``blue'' the skies are. There is no clear definition of population. The geographical location of measurement equipment, the area to cover in the sky, and the time of measurement can be factors in determine the samples. The determination these factors essentially *discretize* the continuous data. After determining all these determining factors succinctly, a technician can make a measurement.
* *Data with Succinct Definition Beyond Reach* We want to study the relationship between the height and weight of the people living in the United States. The population appears to have a clear definition, but it is difficult to determine who gets in the population because the population is transient (due to babies coming to life, people moving out of the country, etc.). By specifying at which point of time the person must be living in the United States, the definition can be succinct. The problem is it is impossible to include everyone in the population. There are more than 300 million people, and we have no way of knowing who those people are.
...
__Systematic or Random, Replace or not Replace, Uniform or Non-uniform__

When we get down to the business of sampling, some critical decisions we must decide before starting to sample.

* The first question is whether to sample systematically or not.
* The second question is whether the same record can appear more than once in the sample data. In the previous chapter, 
* The third is whether the records have equal chances of becoming a sample.


__Systematic Sampling__

In systematic samples, we will choose samples according to a systematic manner.
For example, we can select all students whose university-issued ID number has an odd number in the second-to-last position from the population of all students a university.
Also, we can select, from the entire population of the United States, all voters whose mailing address has the postal code ending in 1, whose street address has a digit 2, owns a car, and whose license plate has either the letter T or the numeral 6.

Here is an example of making systematic sampling on the `mpg` data set using functions on R.
Let's start by preparing a version of the data frame that includes the row index for better visual inspection.
...
This is a combination of systematic sampling and random sampling.
The starting point is random and there are 10 possibilities, and we pick one from these 10 possibilities with probability 10% (i.e., one in ten chances).
The selection after determining the starting point is automatic; we will select every tenth element from the starting point.
Therefore, there are just ten different sample data sets out of the original data set.
Of course there are many other way of selecting one out of ten records from the data set.
Any such combinations will never occur.

Expanding this systematic selection, we can think of splitting the rows in consecutive 10 rows and selecting exactly one from each group.
The systematic selection has more possibilities than the previous one, but still, there are combinations of the same number of records that it will never generates (e.g., the one contains the very first two records).


__Replace or Not Replace__

When you get to make random sampling, there are two major strategies.
One strategy is to prohibit any record to appear more than once in the set.
The other is to not impose the restriction.
We call the first strategy *sampling without replacement* and the second *sampling with replacement*.
Most of the sampling we conduct in this course is sampling with replacement.

__Uniform or Non-Uniform__

There are situations in which the population we access to is a mixture of a few groups, but the representation is not equal among the groups.
For example, if we have a data set covering some students and while the university has a 50:50 mixture of male and female students, the data set has a 20:80 mixture of men and women.
Assuming that both groups must have equal representation, we may want to sample four male students for every female student we sample.


## Empirical Distributions

One major use of sampling is to obtain an approximation for the true distribution.
Think of a question like "how many percent of people in the Unite States are taller than six feet?"
As discussed earlier, we first need to exert a considerable amount of effort in defining "the people in the United States" and then how to collect the height for all qualifying individuals.
The latter action is, as we discussed before, impossible.
We thus turn to sampling; that is, we sample people from the population and approximate the height information from the samples we collect.
Since the question at hand is about the percentage and the attribute in question is a single quanty, we can build a histogram from the height information we collect.
We hope that any height x percentage histogram we generate out of samples will be close to the one we would generate out of the entire population with the same bin set.
Keep in nind that the human height carries inaccuracies because the way a person stands affects height measurement.
...
We can thus imagine having a histogram with 1080 bins with the entire qualifying population and think of the histrograms we generate from sampling is an approximation of this 1080-bin histogram.
We refer to the one with the entire population as the *true distribution* and the one we generate from samples *emprical distributions* (or *observed distributions*).
With the view that the 1080 bins are unsplittable, we cannot increase the number of bins.
...
We take as an example throwing an ideal die.
There are six faces to a die.
The outcome of throwing a die is the face that turns up.
We naturally think of the six numbers in sequence but do not have to do so.
You can go 1, 2, 3, and then 6, 5, 4 because the three numbers appear on the flip side of the first three in that order.
Or, you can go like 2, 3, 1, 6, 5, and then 4.
The six numbers are equal to each other.
We call such a distribution a *discrete distribution*.
The true distribution is the discrete distribution of a fair dice, where the probability of seeing a face is exactly 1/6.
Does that mean that if you throw the die six times, you see each face exactly once?
Not quite.
If that is the case, after throwing the die five times, you must see five difference faces, then you can predict the face of the sixth one to be the one yet to appear.
Also, you can apply the same logic to each consecutive five throws to predict the next one.
The observation woud lead to an assertion that, after the first five throws, the remaining throws are deterministic, which contradicts the randomness we are expecting from the die.

What can we conclude from this rather cumbersome discussion?
We conclude that th ere is no guarantee that the proportions of faces you see after throwing a dice multiple times are not 1/6 even if the number of throws is a multiple of 6.
We can even be bold enough to say that the proportions can be substantially different from the expected "1/6 for each face".

The sampling we will conduct is different from sampling from the human population on three counts.
First, the fair dice may not exist in the real world, and so we will use a computer program that represents a fair die.
Second, the population exists only in our throwing of the die; that is, each time we throw the die, the throw and its outcome becomes a new member of the population.
Third, because we can generate a sample any number of times, the population is inifinite in size.

Now, let us get down to the business of generating samples by throwing a fair die.
...
Here is the ideal discrete distribution of the outcomes of throwing a fair die.
...
Let us visualize the distribution using `ggplot`.
Because there are six values for the face, we use six bars to generate a histogram.
You can see that the height is equal for all bars, at the level of 1/6.
...
Now generate an empirical distribution using computer simulation.
We used previously `sample` for generating samples from a list.
Here will use `dplyr` method for sa,pling with data frames.
The `dplyr` function we use is `slice_sample()`.
It draws at random from the rows of a data frame, with an argument to sample with replacement.
It also gives an an argument `n` for the sample size, and it returns a data frame consisting of the rows that were selected. 
...
We can use `slice_sample` to generate different numbers of samples.
So we can use a variable number of samples, let us define a function for the task of receiving the sample number and generating a distribution plot representing the samples.
The argument `nn` is the sample number.
We could have used a different name for it, e.g., as the plain `n`.
Then we get something like `slice_sample(die, n = n, replace = TRUE)` in the second line.
The two occurrences of `n` have different meanings.
The left one is the name of the parameter the method `slice_sample` uses, and the right one is the argument that the method `empirical_hist_die` receives.
R has no problem in distinguishing the two different meanings of `n`.
Just to be safe, we will use `nn` instead` so as not to confuse ourselves.


```{r}
empirical_hist_die <- function(nn) {
  die_sample <- slice_sample(die, n = nn, replace = TRUE)
...
Here is an empirical histogram of 10 rolls.
It does not look very much like the probability histogram above.
Run the cell a few times to see how it varies.
...
We are talking about 10 throws with six possibilities for each.
Even if try to distribute 10 independent possibilities among the 6 values, it is impossible to generate equal numbers among them.
In the best case, 2 faces get 1 and 4 faces get 2.

When we increase the sample size, we see that the distribution gets closer to the ideal, where each bar has 16.67% as its proportion.
...
The phenomenon we have observed - the empirical distribuition becomes closer to the true distribution as we increase the number of samples - is highly important in data science.
We refer to it as the *law of average*.
A critical requirement for the law of average to be applicable is that the samples come from the same action, irrelevant to other samples.
We call the idea "independently and under identical conditions", sometimes "independently and identically distributed".
