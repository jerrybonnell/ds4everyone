...
Let us now think of the 336,776 flights as a population, and draw random samples from it with replacement. It is helpful to package our code into a function. The function `empirical_hist_delay()` takes the sample size as its argument and draws an empirical histogram of the results.
...
> For a large random sample, the empirical histogram of the sample resembles the histogram of the true population, with high probability.
...
A first step to understanding data from a population is learning about numerical quantities that are associated with it. For instance, in a population of flights that departed New York City (NYC) in 2013, the *median* departure delay can tell us something about the central tendency of the data -- are most departure flights delayed, on time, or are they ahead of schedule? Similarly, in a class of 200 students enrolled in an Organic Chemistry course -- an example that strikes home for many -- the *average* midterm score gives an idea as to what the student population is like. Indeed, these numerical quantities on their own do not explain all there is to know about the underlying populations being studied. Rather, they provide clues as to the kinds of questions we should be asking about the data we are analyzing.         

We call such numerical quantities *parameters* of the population. However, in many situations the parameter is unknown to us. To overcome this, we make use of empirical distributions drawn from large random samples to gauge what that value might look like. Such numbers computed using the data from a sample are called a *statistic*. So we can make an inference about an unknown parameter value by computing a statistic from the empirical distribution -- problem solved? 

### Prerequisites

We will continue to make use of the tidyverse in this section. Our running examples in this section will be flights data from `nycflights13` and National Football League (NFL) data on football players. Let's load in these datasets.  
...
### The statistic can come out differently between runs 

There is one more piece to this story: we observed in the last section that the empirical distribution can look quite different between runs. Recall the histogram of departure delays in `flights`. 
...
Since the population of all departed NYC flights in 2013 is available to us in `flights`, we know the value of the parameter "median flight delay." (lucky us!)  
...
The function `median` returns the half-way point of a column or vector after reordering the values. For `flights`, the returned value means that the median delay was -2 minutes so about 50% of the flights in the population had early departures of 2 minutes or more. 
...
But, what if we tried another random sample of 1,000 flights? Would the statistic come out the same? 
...
Sometimes the sample median is -2, and sometimes it comes out as -1. Therefore, there is variability in the possible values a statistic can take on. This poses a pitfall in our inference approach: if the statistic can come out differently each time depending on the random sample drawn, how could we possibly estimate the unknown parameter?  

One way out of this is to leverage some of the machinery we developed in the previous chapter: simulation! We can simulate the statistic a great number of times and track the values it takes on. A histogram of these values tells us the distribution of the statistic.  

### Simulating a Statistic 

The steps for simulating a statistic follow the same recipe we used for simulating unknown values in the previous chapter. We repeat those steps here in the context of a statistic. 

__Step 1: Determine which statistic to simulate.__ For instance, this could be the median or mean of a random sample of size 1,000 or any other (reasonably large) sample size. 

__Step 2: Figure out how to generate one value of the statistic.__ In this step, we write code to draw a random sample of the size given from Step 1 and compute the statistic of the sample. Typically we encapsulate such steps in a function that we can use as a functional in a `purrr` map. 

__Step 3: Run the simulation and visualize!__ We begin by determining the number of repetitions to repeat the simulation. We then create a vector with length equal to the number of repetitions where all the simulated values will be stored. The map will call the functional we created from Step 2 for each of its elements and store the computed statistic in the corresponding element of the vector. Upon completion, the vector will contain all the simulated values, and we can use this vector for visualization. 
...
### Step 1: Determine which statistic to simulate

We will draw random samples of size 1,000 from the population of flights and simulate the median.  

### Step 2: Figure out how to generate one value of the statistic
...
We will issue 5,000 repetitions of our simulation. Here is the map that makes use of the functional `one_sample_median`. Note that this simulation takes a bit more time to run: we are repeating an experiment where we draw 1,000 random samples a total of 5,000 times! 
...
The sample median is very likely to be about -2, which is exactly the value of the population median. This does not come as surprise: empirical distributions of 1,000 flight departure delays look a lot like the true distribution of flight delays, so it would make sense that the sample median delays would also look a lot like the true median delay in the population. 
...
```{r}
percent_td <- receivers %>%
  pull(is_lg_td) %>%
  mean()
percent_td
```

So about 19.7% of the longest yardages of players ended in a touchdown. How good would our statistic be? 
...
Observe how the sample percentages cluster around the value 0.197, which is the value of the population percentage given by `percent_td` (see the light orange dot). 
