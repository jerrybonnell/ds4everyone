...
The function `empirical_hist_delay()` takes the sample size as its argument, which we call `n`, and draws an empirical histogram of the results.
...
## Aside: The Mean and Median are Meaningful  
...
### The Mean of Two Identical Distributions is the Same 
...
In many situations we do not know the value of a parameter of a population. It turns out that sampling is a reliable tool that can offer a good estimate of a parameter. Since large-scale sampling produces an empirical distribution that is close to the true population, we hope that the value we compute from it will also be close enough to the parameter we obtain from the population when the sample size is large. 
...
* A *statistic* is a value obtained from an empirical distribution (which we can generate). The purpose of a statistic is to *estimate* a parameter. When we compute the mean or median from an empirical distribution, we call these statistics the *sample* mean and median, respectively.  
...
The sample median is very likely to be about -2, which is exactly the value of the population median. This is because the empirical distributions of 1,000 flight departure delays are close to the true ones. We know that we generated before looked a lot like the true distribution of flight delays. We can guess that in running the experiment, each empirical distribution with 1,000 flights would have liked like the true one.
