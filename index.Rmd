...
gutenberg_download <- function(gutenberg_id, mirror = NULL,
  ...
  ids <- if (is.data.frame(gutenberg_id)) gutenberg_id[[1]] else gutenberg_id
  out <- purrr::map_dfr(as.integer(ids), \(id) {
    f <- file.path("data/gutenberg", paste0(id, ".rds"))
    if (!file.exists(f)) stop("No cached Gutenberg text for id ", id, call. = FALSE)
    readRDS(f)
  })
  if (length(meta_fields)) {
    out <- dplyr::left_join(out, dplyr::select(gutenbergr::gutenberg_metadata,
      gutenberg_id, dplyr::all_of(meta_fields)), by = "gutenberg_id")
  }
  out
}
```
...
The online version of the text is updated regularly. Helpful readers have let us know of any typos or errors they found in past versions. Please continue to [let us know](https://forms.gle/c3YgzCPkk3HQx7b2A) about such things.
