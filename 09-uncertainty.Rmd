...
There are cases, by the way, where two medians exist. Such an event occurs exactly when the number of values is an even number. If there are 10 values, the 5th and the 6th values in the ascending order are the two medians. The one at the lower position in the order is the *odd median* and the other one, i.e., the one at the higher position in the order, is the *even median*. To compute the median in this case, we usually take the average of the odd and even median.  
...
ggplot(df, aes(x = bstrap_means, y = after_stat(density))) + 
  geom_histogram(col="grey", fill = "darkcyan", bins = 8) +
  annotate("segment", x = middle95[1], y = 0, 
...
ggplot(df, aes(x = bstrap_means, y = after_stat(density))) + 
  ...
  annotate("segment", x = middle95[1], y = 0, 
                   xend = middle95[2], yend = 0, 
                   size = 2, color = "salmon") +
  annotate("point", x = pop_mean, y = 0, color = "red", size = 3)
```
...
ggplot(df, aes(x = bstrap_means, y = after_stat(density))) + 
  ...
  annotate("segment", x = middle[1], y = 0, xend = middle[2], 
           yend = 0, size = 2, color = "salmon") +
...
### Confidence intervals as a hypothesis test
