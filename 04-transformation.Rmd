...
When a dataset is expressed in this manner, we say that it is in *long* format because the number of rows is comparatively larger compared to `bakery1` and `bakery2`. Admittedly, this form can make it harder to identify patterns or trends in the data by eye. However, tidy data opens the door to more efficient data science so that you can rely on existing tools to do the things you need to do. Without a standardized means of representing data, such tools would need to be developed from scratch each time you begin work on a new dataset.   

Observe how this dataset fulfills the four properties of tidy data. The fourth property is fulfilled because the observational unit we are measuring is a weekly cookie sale, and we are measuring three variables -- `week`, `type`, and `sales` -- per observational unit. These variables do not refer to measurements on a sale or bakery store; they refer to measurements on a given weekly cookie sale from a local Miami bakery in 2021. 

Violations of the third property are peculiar and can be rare. A possible violating scenario could occur if the bakery decides to record sale *ranges*, e.g., in the case of making a forecast on future sales. 
...
The fourth principle of tidy data stated that an observational unit should form a table. However, often times the observational unit we are measuring will be split across multiple tables.  
...
The main construct we will be using from `purrr` is something called the *map*. A map applies a function, say the `passes_100()` function we wrote earlier, to each element of a vector or list. 
