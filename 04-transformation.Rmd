...
knitr::include_graphics('images/dplyr_verbs.svg')
```
...
* _A function may take upon the role of computing a value._ You can design a function so that it uses a special function `return` at the end so as to specify the value it has computed. Note that the use of `return` is __optional__ and, by default, R always returns the last line of computation performed in the function. 
...
We could write the above anonymous function more compactly using a `purrr`-style "formula". 
...
```{r, echo=FALSE, fig.align="center", out.width='40%', fig.asp=1/2}
knitr::include_graphics('images/csc100-Page-2.drawio.svg')
```
...
## Case Study I: A Computer Science Course

In this section, we present a worked example of a real-life dataset and apply the transformations that are necessary to bring it into tidy data. The goal of this exercise is to demonstrate how the transformations we have learned in this chapter are not tools meant to be used in isolation, but work best when used together to fulfill a complex goal.  
...
## Case Study II: Department of Education College Scorecard
...
The full data is available for download [here](https://collegescorecard.ed.gov/data), however, we have already done some basic preprocessing of this dataset by including only Florida-based institutions. This tibble is available in `scorecard_fl`. Let us have a look at the data:

```{r echo=FALSE}
scorecard_fl <- read_csv("data/Most-Recent-Cohorts-Institution-FL.csv") 
``` 
...
We can remedy the problem by converting all instances of `"NULL"` to proper missing values so that these can be detected appropriately by our `dplyr` verbs. 
...
1. What is the average net tuition for public and private institutions in each zip code area? Which ZIP codes see the highest average net tuition?
2. Are private institutions more expensive to attend in the Miami area? 
3. Which institutions are below the average net tuition for that ZIP code area? 
