...
We will modify the above code to add one more column, `prod`, that takes the product of the columns `x_su` and `y_su`. We will also save the resulting tibble to a variable called `linear_df_standard`.
...
One way to make a prediction about the outcome of some individual is to first find others who are similar to that individual and whose outcome you do know. We can then use those outcomes to guide the prediction. 

Say we have a new black cherry tree whose height is 75 ft. We can look at trees that are "close" to 75 ft, say, within 1 foot. We can find these individuals using `dplyr`:
...
We can take the mean of the outcomes for those observations to obtain a *prediction* for the *average* diameter of a black cherry tree whose height is 75 ft. Let us amend our `dplyr` code to compute this value. 
...
toy_df %>% 
  ...
  geom_line(aes(x = x, y = y),
            color = "darkcyan",
            size = 2) + 
  annotate("point", x = 1, y = my_quadratic_equation(1),
             color = "salmon",
             size = 4) 
```
...
ggplot(girth_height_su) + 
  ...
  geom_vline(xintercept = 1, color = "blue") + 
  annotate("point", x = -1, y = 0, size = 2, color = "blue") + 
  annotate("point", x = -1, y = -0.5, size = 2, color = "blue") + 
  annotate("point", x = -1, y = -1, size = 2, color = "blue") + 
  annotate("point", x = 1, y = 0, size = 2, color = "blue") + 
  annotate("point", x = 1, y = 0.5, size = 2, color = "blue") + 
  annotate("point", x = 1, y = 1, size = 2, color = "blue") + 
  geom_segment(aes(x = Height_su, y = Girth_su,
...
ggplot(bstrap_predictions, 
  ...
  annotate("segment", x = middle[1][[1]], y = 0, 
...
`predict` obtains the interval by means of statistical theory, and we can see that the result is very close to what we found using the bootstrap. Formally, these intervals go by a special name: *confidence intervals for the mean response*. That's a mouthful!
...
* **Question 1.4** Are the percentile of sugar and the overall win percentage roughly linearly associated?  Do you observe a correlation that is positive, negative, or neither? Moreover, would you guess that correlation to be closer to 1, -1, or 0? 
