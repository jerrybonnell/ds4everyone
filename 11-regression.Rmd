...
In this chapter we turn to making guesses, or *predictions*, about the future. There are many examples in which you, your adviser, or an employer would want to make claims about the future -- that are accurate! For instance: 
...
We will make use of the tidyverse in this chapter, so let’s load it in as usual. We will also return to some datasets from the `datasauRus`, so let us load that in as well. 
...
Compare the line we have just found (in orange) with the line $y = x$. For trees with heights that are *less* than 0 $SD$s away from the mean, the orange line predicts a *larger* girth than the $y = x$ line. And, for trees with heights that are *more* than 0 $SD$ away from the mean, the orange line predicts a *smaller* girth than the $y = x$ line. The effect should become clearer in the following plot. 
...
* Resample the samples that are present in `mlb` by means of sampling with replacement. 
...
```{r}
one_resample <- function(df, x, y, x.new) {
...
```{r}
one_resample(mlb, "Height", "Weight", tibble(Height = c(72)))
```

Let us now repeat the bootstrap a large number of times, say 10,000 times. 

```{r}
bstrap_predictions <- replicate(n = 10000, one_resample(mlb, "Height", "Weight", tibble(Height = c(72))))
```
...
Let us plot an sampling histogram of the predictions and annotate the confidence interval on this histogram.
...
```{r}
one_resample(mlb, "Height", "Weight")
```

As before, we will repeat the process 10,000 times. 

```{r}
bstrap_slopes <- replicate(n = 10000, one_resample(mlb, "Height", "Weight"))
```
...
Following is the sampling histogram of the bootstrapped slopes with the annotated confidence interval. We also plot a red dot representing the original slope, which we found to be $4.841$.   
