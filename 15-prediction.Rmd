...
$$
and make the total of the squared residual values as small as possible.
There is a mathematical method for obtaining the setting to the parameter values that achieves the minimum *total squared distance* when there is only one factor, but learning such methods is beyond the scope of this course.

## Correlation

To explain how a linear regression estimator in R works, let us use an artificial data set.
...
The statistical concept we use for the investigation is *correlation coefficient*.
...
We usually say *correlation*, omitting the "coefficient*.
...
R comes the function for computing the correlation. You have only two provide the two sources for which you need to compute the correlation.
...
### An artificial data set with random points

Let us look at the calculation of correlation a little bit more.
We will define a function `pop.sd` and our own version of `scale` called `pop.scale`, as `scale` makes use of `sd`.
...
Because of randomness in the simulation, the correlation is not expected to be exactly equal to  $r$.
Let us try calling `r_scatter` with different values of $r$.
...
The scatter plot looks like an elipses that is perfectly linear and slopes upward.
Below appear five call with $r = 0.95, 0.25, 0, -0.5$, and $-1$, respectively.

Observe that when $r = 0$, the scatter plot is a formless cloud around the horizontal axis, and the variables are said to be *uncorrelated*.
...
Let us use the data set `dt` from the previous discussion.
...
Using the coefficients we obtain the linear function.

$$
f(xx) = 2.0944 + 24.9283 * xx
$$
...
The `call` attribute shows the arguments of `lm`.

```{r}
summary(fitted)$call
```
The `coefficients` attribute shows the coefficients of the model.

```{r}
summary(fitted)$coefficients
```

These eight numbers appear as a single list in `coefficients`.
They appear in the column-major order.
values 
```{r}
summary(fitted)$coefficients[1]
summary(fitted)$coefficients[2]
summary(fitted)$coefficients[3]
summary(fitted)$coefficients[4]
summary(fitted)$coefficients[5]
summary(fitted)$coefficients[6]
summary(fitted)$coefficients[7]
summary(fitted)$coefficients[8]
```
The `Std.error` shows the *standard error* (which we will not discuss in this course).
The standard error is a representation of the variability of estimates.
Its appearance here in the table informs that the function `lm` produces a model through sampling.
We observe that the standard error is high for the intercept and low for the coefficient on `xx`.

The `residuals` attribute shows the difference of the actual value and the estimate for all data points.
In the present example, this is `ww` minus the function $f$ at `xx`.
We observe that there are 41 difference values appearing in `residuals`.

```{r}
summary(fitted)$residuals
```

Another important attribute is `r.squared`.
```{r}
summary(fitted)$r.squared
```

What is the `r.squared` attribute?
It is the $R^2$ value of the regression, which we compute as follows:

* Compute the residual for each observation.
* Compute the difference of each `yy` from the the average of all `yy` values.
* Compute the sum of the square of the residuals.
* Compute the sum of the square of the difference values.
* Compute the ratio of the first sum to the second sum.
* Subtract from 1 the ratio.

The result of the last subtraction is the $R^2$ value.

$$
1 - \frac{\mathrm{sum~of~the~square~of~the~residual}}{\mathrm{sum~of~the~square~of~the~distance~from~the~average}}
$$

Let us compute the $R^2$ value ourselves.
Below, we obtain the parameters of the model as the values of `ccc` and `aaa`.

```{r}
ccc <- summary(fitted)$coefficients[1]
aaa <- summary(fitted)$coefficients[2]
ccc
aaa
dt %>% mutate(fit = ccc + aaa * xx) %>% mutate(res = ww - fit) %>% mutate(res2 = res * res) %>% mutate(diff = ww - mean(ww)) %>% mutate(diff2 = diff * diff) -> dt2
r2 <- 1 - sum(dt2$res2) / sum(dt2$diff2)
r2
```

We observed that the manual calculation matches the `r.squared` attribute.

### Regression without intersept

We can force the modeling function to keep the intercept to be 0 by adding `-1` to the formula when calling `lm`.


```{r}
fitted2 <- lm(data = dt, ww ~ xx - 1)
summary(fitted2)
```
...
```{r}
summary(fitted2)$r.squared
summary(fitted2)$coefficients
summary(fitted2)$coefficients[1:2]
```
We observe that the value of `r.squared` is higher than the previous one.

## Height and weight relations
...
```{r}
ggplot(mlb, aes(x = Height, y = Weight)) + geom_point(color="darkcyan") + geom_smooth()
```

The data has one missing value.
We can compute the correlation with the missing value omitted.
The option `use = "pairwise.complete.obs"` accomplishes this.

```{r}
cor(mlb$Height,mlb$Weight,use="pairwise.complete.obs")
```

Let us use R to compute the regression.
In our case, there is only one factor, `Height`, so we state `Height` in the place for `FORMULA`.

```{r}
fitmlb <- lm(data = mlb, Weight ~ Height)
```

Let us look at the `r.squared` and `coeffients`.

```{r}
summary(fitmlb)$r.squared
summary(fitmlb)$coefficients
```

While the correlation was about $0.50$, if you run linear regression, the resulting $R^2$ value is only $0.2829", not very high.

Let us compute the standard deviation of the residual values using `sd()` and then plot the estimated line along with two lines below and two lines above.
The four lines are parallel to the estimated line with different intercept values.
The lines immediate above and immediate below are with intercepts higher and lower by the standard deviation, respectively; the other two lines, the difference is two times the standard deviation.
We will include the additional four estimates.
It is possible to generate the five (estimate, two above, and two below) within `ggplot`, but here we compute them and add the values as columns.

```{r}
ssd <- sd(summary(fitmlb)$residual)
ssd
cmlb <- summary(fitmlb)$coefficients[1]
amlb <- summary(fitmlb)$coefficients[2]
mlb %>% mutate(est = cmlb + amlb * Height) %>% mutate(upper = est + ssd) %>% mutate(lower = est - ssd) %>% mutate(upper2 = upper + ssd) %>% mutate(lower2 = lower - ssd) -> mlb0
head(mlb0)
```

The standard deviation is $17.77$ (in pounds), which appears quite high.

Here we plot the lines.

```{r}
ggplot(mlb0, aes(x = Height, y = Weight)) +
  geom_point(color="darkcyan") +
  geom_line(aes(y = est), color = "black") +
  geom_line(aes(y = upper), color = "blue") +
  geom_line(aes(y = lower), color = "red") +
  geom_line(aes(y = upper2), color = "blue") +
  geom_line(aes(y = lower2), color = "red")
```

It looks like the predicting weight from height is not so successful.

Here is another plot, instead 

```{r}
rename(mlb0, observed = Weight, estimated = est, plus2xSD = upper2, minus2xSD = lower2) %>%  pivot_longer(c(observed, estimated, plus2xSD, minus2xSD), names_to = "measure", values_to = "Weight") -> mlb1

ggplot(mlb1) + 
  geom_point(aes(x = Height, y = Weight, color = measure), alpha = 0.5) + 
  scale_color_manual(values = c("red", "purple", "darkcyan", "blue"))
```

## Exploring the `mtcars`

Let us return to the `mtcars` data set, which is the collection of car specs.
...
Let us compare the attributes with `mpg` concerning correlation.
We can use `paste()` to add a prefix to the correlation value.

```{r}
paste("cyl:  ", cor(mtcars$cyl,mtcars$mpg))
paste("disp: ", cor(mtcars$disp,mtcars$mpg))
paste("hp:   ", cor(mtcars$hp,mtcars$mpg))
paste("drat: ", cor(mtcars$drat,mtcars$mpg))
paste("wt:   ", cor(mtcars$wt,mtcars$mpg))
paste("qsec:", cor(mtcars$qsec,mtcars$mpg))
paste("vs:   ", cor(mtcars$vs,mtcars$mpg))
paste("am:  ", cor(mtcars$am,mtcars$mpg))
```

It appears that "drat", "qsec", "vs", and "am" have low correlation values, so let us ignore these four to create a new data set.

```{r}
mtcars1 <- mtcars %>% select(mpg, cyl, disp, hp, wt)
head(mtcars1)
```

```{r}
ggplot(mtcars1, aes(x = cyl, y = mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = disp, y = mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = hp, y = mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = wt, y = mpg)) + geom_point() + geom_smooth()
```

Let us use the five attributes to attempt to estimate `mpg.

```{r}
fitted_qsec <- lm(mpg ~ cyl, data = mtcars1)
summary(fitted_qsec)$r.squared
summary(fitted_qsec)$coefficients
```

```{r}
fitted_disp <- lm(mpg ~ disp, data = mtcars1)
summary(fitted_disp)$r.squared
summary(fitted_disp)$coefficients
```

```{r}
fitted_hp <- lm(mpg ~ hp, data = mtcars1)
summary(fitted_hp)$r.squared
summary(fitted_hp)$coefficients
```

```{r}
fitted_wt <- lm(mpg ~ wt, data = mtcars1)
summary(fitted_wt)$r.squared
summary(fitted_wt)$coefficients
```

Here is the one in which all five can be factors.
The value of $R^2$ is much higher than any of the four individual ones.

```{r}
fitted_all <- lm(mpg ~., data = mtcars1)
summary(fitted_all)$r.squared
summary(fitted_all)$coefficients
```

Let us remember the $R^2$ value as the best so far.

```{r}
r2best <- summary(fitted_all)$r.squared
```

Is it possible to account for the curves appearing in the smooth plots with `disp`, `hp`, and `wt`?
Let us consider the inverse of the three quantities, which we can express as `1/disp`, `1/hp`, and `1/wt`.

```{r}
ggplot(mtcars1, aes(x = 1/disp, y =mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = 1/hp, y = mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = 1/wt, y = mpg)) + geom_point() + geom_smooth()
```

In addition, let us think of the logarithm function.
The logatihm asks how many times you need to multiply 2 to produce the value at hand.
We often use 10 in place of 2.
For example, the logarithm of 2 base 2 is 1, the logarithm of 4 base 2 is 2, the logarithm of 8 base 2 is 3, etc.
Let us consider `log(disp)`, `log(hp)`, and `log(wt)`.

```{r}
ggplot(mtcars1, aes(x = log(disp), y =mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = log(hp), y = mpg)) + geom_point() + geom_smooth()
ggplot(mtcars1, aes(x = log(wt), y = mpg)) + geom_point() + geom_smooth()
```

It looks like the six values have the effect of flattening the curve.
Let us put all the six values as attributes to create a new data set and then run linear regression.

```{r}
mtcars1 %>% mutate(invdisp = 1/disp) %>% mutate(invwt = 1/wt) %>% mutate(logdisp = log(disp)) %>% mutate(logwt = log(wt)) -> mtcars2
fitted2 <- lm(data = mtcars2, mpg ~ .)
summary(fitted2)$r.squared
summary(fitted2)$coefficients
```

The $R^2$ value is now over 0.90.
Use these coefficients to define the line.
We call it `est`.

```{r}
coeff <- summary(fitted2)$coefficients
mtcars2 %>% mutate(est = coeff[1] + coeff[2] * cyl + coeff[3] * disp + coeff[4] * hp + coeff[5] * wt + coeff[6] * invdisp + coeff[7] * invwt + coeff[8] * logdisp + coeff[9] * logwt) -> mtcars3
```

Let us plot the `est` and `msg` together.
The red line below is `msg` against `msg`.
The blue line is `est`.

```{r}
ggplot(aes(y = est, x = mpg), data = mtcars3) +
...
### The correlation coefficient (OLD)
...
### Association is not Causation (OLD)
...
### Correlation Measures *Linear* Association (OLD)
...
### Correlation is Affected by Outliers (OLD)
...
### Ecological Correlations Should be Interpreted with Care (OLD)
...
### Serious or tongue-in-cheek? (OLD)
...
### Measuring in Standard Units (OLD)
...
### Identifying the Line in Standard Units (OLD)
...
### The Regression Line, in Standard Units (OLD)
...
### The Equation of the Regression Line (OLD)
...
### The Regression Line and Galton's Data (OLD)
...
### Units of Measurement of the Slope (OLD)
...
## The Method of Least Squares (OLD)
...
### Minimizing the Root Mean Squared Error (OLD)
...
### Detecting Heteroscedasticity (OLD)
