...
0.0112 0.089 0.0115 0.0176 0.0217 0.0182
\]
The sum of the absolute differences is 0.0891.
...
Let us look at the proportion of the elements in the list `diffs` that are at least the observed diff value 0.0892, whose value we have stored in `observed_point`.
...
The value we get is 0.065. <!-- how do we adjust according to the outcome? -->
...
Then the chances that the difference achieves 0.065 that we observed are no more than 6.5%.
We can thus conclude that the chances that the die at hand is a fair one are no more than 6.5%.

The value we have calculated, 0.065, through simulation is a p-value.
...
Using simulations we learn that the Null Hypothesis has a p-value of 6.5% (or 0.065) using the absolute distance between distributions as the distance measure.
