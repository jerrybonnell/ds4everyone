...
```{r}
one_simulation <- function(x, df, label, statistic, num_draws) {
  label <- enquo(label) # Create quosure
  
  sample_df <- slice_sample(df, n = num_draws, replace = TRUE)
  results <- statistic(pull(sample_df, !!label))
  return(results)
}
```
...
simulate_sample_mean <- function(sample_size, repetitions) {
  ...
  print(paste("Population mean: ", sample_size))
}
```
