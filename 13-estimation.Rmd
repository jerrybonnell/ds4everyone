...
As we mentioned earlier, the maximum, the minimum, and the medians are part of what we call "order statistics".
Order statistics are values at certain positions in numerical data.
We expand the concept of order statistics in this chapter and learn about percentiles and quartiles.
We then learn a very important technique in data science *bootstrapping* to produce distributions from a sample.
We apply the order statistics to the distributions we generate using bootstrapping and obtain confidence intervals.

## Ranking and Order Statistics

Let us start by loading the library `tidyverse`.

```{r}
library(tidyverse)
```

We previously used a National Football League receiver data set.
This time, we will use an NCAA receiver data set covering the years 2005 - 2011, which is available at [Football Outsiders](https://www.footballoutsiders.com/college-xp/2012/2005-11-college-football-receiver-data-targets-and-catches).
...
```
Here is the list of attributes of the data. 

```{r}
spec(ncaa0)
```

The attributes are slightly different from the National Football League data.
The performance attributes are in three groups: overall, standard, and passing.
An offensive turn has at most four plays (which we call "downs").
The goal of an offensive turn is to either bring the ball to an area beyond the goal line of the opponent (a "touchdown") or move forward by at least 10 yards.
If the offensive team is successful in accomplishing the 10-yard-forward goal, the team gues to play offense again.
The strategy to use depends on how many yards the team has to move the ball forward and how many downs remaining.
The "standard" situations are: the first down, the second down with at most 7 yards towards the 10-yard-forward accomplishment, the third down with at most 4 yards to go, and the fourth down with at most 4 yards to go.
The other situations are "passing".
In "passing" situations, the best strategy for the offensive team is to have the quarterback throw the ball to a receiver since throwing, if successful, can gain many yards.
In "standard" situations, in addition to a throwing play, the team can try to move the ball forward by having a player carry the ball forward.
The defense team, whose main goal is to prevent the defense team from moving the ball forward, has to anticipate a play to happen and act according to the play that emerges.
In a "passing" play, the defense team can position its players in anticipation of a throw to happen, while in a "standard" play, the team may be ready for both a throw and a run.
The data shows the receiver performance in "standard" and "passing" downs separately, and overall. 


As in the case of the National Football League data, the same receiver may appear more than once and so create an attribute that combines the player name and the year using `mutate`.
...
What are the maximum and minimum of the yardage data?
How about the average?
Who achieved the maximum value?

```{r}
nrow(ncaa)
max(ncaa$Yards)
min(ncaa$Yards)
sum(ncaa$Yards)/length(ncaa$Yards)
maxy <- max(ncaa$Yards)
ncaa %>% filter(Yards == maxy)
```

The data set has 7323 rows, and the maximum and the minimum of the yardage attribute is 1962 and 6, respectively.
The average is 299.9 yards.
[Michael Crabtree](https://sports.yahoo.com/nfl/players/9274/), who presently (in 2020) plays for the Arizona Cardinals achieved the record in 2007 as a Texas Tech receiver.

Here is the distribution of the yardage of the players.
We observe that the distribution stretches to the right.


```{r dpi=80,  fig.align="center", message = FALSE}
ggplot(ncaa, aes(x = Yards, y = ..density..)) + 
  geom_histogram(col="grey", breaks = seq(0, 2000, 10))
```

Let us look at the peak area, which is somewhere below 200.

```{r dpi=80,  fig.align="center", message = FALSE}
ncaa1 <- ncaa
filter(ncaa1, Yards < 200)
ggplot(ncaa1, aes(x = Yards, y = ..density..)) + 
  geom_histogram(col="grey", breaks = seq(0, 200, 10))
```

So the peak is much smaller than the average.

### Percentile

Percentile is an order statistic where the position of the data is not the rank but a percentage that specifies relative position in the data.
The advantage of using percentile is that the position specification does not directly take into account the data size.
What do we mean by that?
Suppose we have two lists of exam scores, `listA` with scores of 50 students and `listB` with scores of 4000 students.
Think of "top 10" in each data set.
Since 10 is 20% of 50 elements, there are 20% chances for a student to be among the top 10 group in `listA`, while the chances are 0.25% in `listB`.
That is, if we specify a top group with its size, the significance being in the top group varies depending on the size of the population and so we must to specify the size of the underlying group, e.g., "top 10 in a group of 4000 students".

Percentile is a concept that eliminates the use of population size in specifying the position.
We simply represent the position its relative position to the entire population.
Instead of saying "top 10 in a group", we say top 20%.
Two strategies exist in specifying percentile: starting from the top and starting from the bottom.
We specify the former by attaching the word "top" and the latter "bottom".
The bottom percentile is more common than the top percentile.
The bottom percentile corresponds to mearing the distance from the very left end of the distribution.

Let us look at some percentile values in the yardage data.

```{r}
yardage <- ncaa$Yards
sortedyards <- sort(yardage)
percentile <- function(x) {
  return(sortedyards[x*length(sortedyards)/100])
}
```

Let us examine some percentile values.
...
percentile(100)
```
Tne 50 percentile point is the median of the data.
The median yardage is 207 yards.
The last four values demonstrate how long the stretch of the data is.
The 90 percentile is only 678, the 95 percentile is 859, the 99 percentile is 1210, the 99.9 percentile is 1911, and the 100 percentile is the maximum 1962.
The gap between the 99.9 percentile and the 100 percentile is a staggering 340 yards.
It appears how an amazing player Michael Crabtree was as a college football player.

The percentile does not exist for 0%.

```{r, error=TRUE}
percentile(0)
```
We can obtain the top percentile using 100 minus the percentile value in question.

### Quartile and Range Statistics

In addition to the medians, important percentiles are the 1/4 and 3/4, which we often call the bottom quarter and the top quarter.
Basically, we chop the data in quarters and use the boundaries between the neighboring quarters.
The quartiles and the median are as follows:
...
Let's look at a different quantity.
`YdsPerCatch` is the average number of yards the receiver advanced the ball with his catch.


```{r dpi=80,  fig.align="center", message = FALSE}
ggplot(ncaa, aes(x = YdsPerCatch, y = ..density..)) + 
  geom_histogram(col="grey", breaks = seq(0, 40, 1))
```

The distribution looks more symmetric than the yardage, with the peak at 12.
Let us define a function for computing the percentile for `YdsPerCatch`.
We will obtain the max, the min, the mean, and the percentiles at 10, 25, 50, 75, and 90.

```{r}
sorted_ypcatch <- sort(ncaa$YdsPerCatch)
percentile_ypcatch <- function(x) {
  return(sorted_ypcatch[x*length(sorted_ypcatch)/100])
}
min(sorted_ypcatch)
max(sorted_ypcatch)
mean(sorted_ypcatch)
percentile_ypcatch(10)
percentile_ypcatch(25)
percentile_ypcatch(50)
percentile_ypcatch(75)
percentile_ypcatch(90)
```

The maximum of 43 is quite stunning, given that it is even difficult to advance by 10 yards in four downs.

By combining two percentiles, we can get a rough sense of the distribution.
For example, the combination of 25% and 75% percentiles represents the "middle" 50%.
Similarly, the 37.5% and 62.5% percentiles jointly represent the middle quarter, and the 45% and 55% jointly represent the middle 10%.
The way we arrive at 37.5 and 62.5 is to add/subtract one half of 25 to/from 50.
The way we arrive at 45.0 and 55.0 is to add/subtract one half of 10 to/from 50.

```{r}
c(percentile_ypcatch(25),percentile_ypcatch(75))
c(percentile_ypcatch(50-25/2),percentile_ypcatch(50+25/2))
c(percentile_ypcatch(50-10/2),percentile_ypcatch(50+10/2))

```

By the way, who achieved the heihgt yards per catch?


```{r}
maxy <- max(ncaa$YdsPerCatch)
ncaa %>% filter(YdsPerCatch == maxy)
```

Interestingly, the player Darius Williams became the target only 13 and caught the ball only three times.
How did Michael Crabtree do?

```{r}
ncaa %>% filter(Player == "Michael Crabtree")
```
Michael Crabtree has two entries in 2007 and in 2006, where his yards per catch is 14.6 and 12.0, respectively.
...
Sometimes a data scientist receives a collection of samples from the underlying population to which she has no access.
If she had access to the underlying population, she might attempt to generate an array of samples, from which she would generate a distribution of the statistic of interest.
Since that is impossible, is there a way for to deal with the sample collection at hand to generate a range of values for the statistic?
Yes, we call that *resampling* or *bootstrapping*.
In resampling (or bootstrapping) you treat the data set at hand as the true population and generate samples from them.
But there is a catch.
Each sample data set you generate should be equal in size to the original you have.
In stead of sampling without replacement, you use sample with replacement.
Since the sample data sets have the same size as the original, with the use of replacement, you get duplicates and omissions; that is, there are items appearing multiple times in and there are items missing.
Because of the use of randomness, the discrepancy varies.
...
The resampling idea resembles a situation in which there are 365 people instead of 23.
...
The value $e$ is what we call *Euler's constant* and is about 2.718, and so for a large enough $B$, the ratio is quite small.
Thus, the analysis guarantees that the samples we generate practically will not look like the original.

### Getting ready

Let us start with loading the `tidyverse` package and loading the NCAA data.
...
Let us create a subset of the data mechanically.
We will select Year 2006.
There are 1004 receivers appearing in the subset.

```{r}
ncaa1 <- filter(ncaa, Year == 2006)
nrow(ncaa1)
yards <- ncaa1$YdsPerCatch
```

Assume this is the population we have at hand.
The method for generating a subset of an equal size with replacement can be the following.
...
Using `resample`, we can generate a series of bootstrapped data set and generate the distribution of the averages of the data.

```{r}
averages <- vector("double", 500)
for (i in 1:500) {
  averages[i] <- sum(resample(yards))/length(yards)
}
averages <- sort(averages)
```

Let us also define again the function that returns the percentile.

```{r}
percentile <- function(x, y) {
  return(x[y * length(x) / 100])
}
```

Here are the minimum, maximum, and the quartiles of the averages.

```{r}
averages[1]
percentile(averages, 25)
percentile(averages, 50)
percentile(averages, 75)
percentile(averages, 100)
...
```{r dpi=80,  fig.align="center", message = FALSE}
df <- tibble(averages)
ggplot(df, aes(x = averages, y = ..density..)) + 
  geom_histogram(col="grey", breaks = seq(11, 13, 0.05))
```

The 90% range of the averages is
```{r}
the_range <- c(percentile(averages,5),percentile(averages,95))
```

In other words, this is the range that covers 90% of the yards per catch values of the receivers.

Now we can check how many of the receivers in the true population falls in this region.

```{r}
ncaa2 <- filter(ncaa, YdsPerCatch >= the_range[1] & YdsPerCatch <= the_range[2])
nrow(ncaa2) / nrow(ncaa)
overall_mean <- mean(ncaa$YdsPerCatch)
```

Let us see the distribution of the averages of the 2006 data in contrast with the mean of the means (in green) and the overall mean (in red).

```{r dpi=80,  fig.align="center", message = FALSE}
df <- tibble(averages)
mean_of_means <- percentile(averages,50)
ggplot(df, aes(x = averages, y = ..density..)) + 
  geom_histogram(col="grey", breaks = seq(11, 13, 0.05)) +
  geom_point(aes(x = overall_mean, y = 0), color = "red", size = 3) +
  geom_point(aes(x = mean_of_means, y = 0), color = "green", size = 3)
```
...
The subset selection that we used `Year == 2006` was very artificial because there may have been some annual bias on the data in 2006.
The gap between the red and the green dot is an indication of such a bias.
So, let us try again.
This time, we select 1000 data points, about the same number as the 2006 data, by random sampling.

```{r}
yards2 <- sample(ncaa$YdsPerCatch, size = 1000, replace = TRUE)
mean(yards2)
```

We then perform the series of definitions and operations as we did before.
We use different names for some data: `yards` to for the 1000 subset of the yardage column and `averages2` for the means we generate from bootstrapping.

```{r}
resample <- function(x) {
  sample(x, size = length(x), replace = TRUE)
}
averages2 <- vector("double", 500)
for (i in 1:500) {
  averages2[i] <- sum(resample(yards2))/length(yards2)
}
averages2 <- sort(averages2)
```
...
df2 <- tibble(averages2)
mean_of_means <- percentile(averages2,50)
overall_mean <- mean(yards2)
ggplot(df, aes(x = averages2, y = ..density..)) + 
  geom_histogram(col="grey", breaks = seq(11, 13, 0.05)) +
  geom_point(aes(x = overall_mean, y = 0), color = "red", size = 4) +
  geom_point(aes(x = mean_of_means, y = 0), color = "green", size = 2)
```

To see more clearly the two dots, we use size 4 for the red dot and use size 2 for the green dot.
The order we plot the two is red first, and so the small dot in green will surely appear on top of the red dot without completely covering the red.
We see that the two dots overlap with each other.
Our conclusion from the second experiment is that the true overall average must be sitting in the interval between the 5 percentile and 95 percentile with 90% confidence.
See yourself by examining the values.

```{r}
c(percentile(averages2,5),percentile(averages2,95))
overall_mean
```

### Confidence Intervals

We call the interval we used to encompass the mean of the means a *confidence interval*.
We attach the size of the coverage of the interval as an attributed of the interval, e.g., 90% confidence interval, 95% confidence interval, etc.
A confidence interval means that by way of resampling (or some other method) we are "confident" that the true value must be sitting in the interval.
So, the 
...
ncaa <- read_csv("data/ncaa-reivers-2005-2011.csv")
```
