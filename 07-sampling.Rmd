...
tibble(some_numbers) |>
  ...
  geom_histogram(aes(x = some_numbers, y = after_stat(density)),
                 color = "gray", fill = "darkcyan", binwidth = 1) +
  annotate("point", x = mean(some_numbers), y = -0.02, 
...
annotate("point", x = mean(symmetric), y = -0.02, 
           color = "salmon", shape = 17, size = 4)
```
...
* A *statistic* is a value obtained from a sampling distribution (which we can generate). The purpose of a statistic is to *estimate* the parameter of interest. When we compute the mean or median from a sampling distribution, we call such statistics the *sample* mean and median, respectively.  
