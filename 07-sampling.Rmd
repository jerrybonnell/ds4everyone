...
### Probabilistic sampling and random samples
...
This is a combination of systematic sampling and probabilistic sampling. The starting point is random and there are 10 possibilities, and we pick one from these 10 possibilities with probability 10% (i.e., one in ten chances). The selection after determining the starting point is deterministic; we select every tenth element from the starting point. Therefore, there are just ten different sample data sets out of the original data set.
...
Despite the use of the random initial selection, we caution against classifying this sample as a proper *random sample*. The systematic selection of every 10 rows after the initial random choice may disproportionately favor one or more groups from the population. Without further consideration, we would be right to label this a *convenience* sample and not a random sample. Convenience samples are examined in greater depth in [Section 7.6](https://ds4world.cs.miami.edu/power-of-sampling.html#sampling-plans).  
...
Most of the sampling we conduct is sampling *with* replacement. The reasons for this will follow in the next section.   
...
```{r}
slice_sample(die, n = 6, replace = FALSE)
```
...
Since we are assuming that the population of all departed NYC flights in 2013 is available to us in `flights`, we can directly compute the value of the parameter "median flight delay." This is a rare luxury not often possible using real data. We can check the value we obtain through sampling against the value we obtain from the entire population.
...
You can run the code a few times to see that the value that appears as the result is not consistent, but always either -1 or -2. While the two values are close, you cannot assert that the median is -1 or it is -2. 

Is there any way out of this? Can we say anything about how likely the median is -1 and how likely the median is -2? We thus turn to simulation. We obtain the statistic of median many times using sampling and then see a histogram of the numbers we obtain.
...
Before continuing with the `flights` data, let us see an example of simulating a statistic using a toy problem. 

Your friend invites you to a game of rolling die. He rolls a special 30-sided die (sometimes called a "D30" die) 10 times and, after each roll, adds some "lucky" number of his choosing. He does not say what his lucky number is. 

Let us suppose the lucky number is 6. We can use `sample` to simulate the total rolls we get to see. This is what one set of total rolls might look like after the game is over:
...
This mean goes by a special name: the *expected* value. Statisticians gave it this name because it is the number that we can *expect* to get in the long run, say after doing an arbitrarily large number of trials. For a 30-sided die, probability theory dictates this should be 15.5. Our answer in `d30_expected_value` comes pretty close!
...
We put the results together into a tibble `estimate_tibble`. Note that a pivot transformation is applied here such that one row corresponds to one simulated statistic. 
...
```{r dpi=80, fig.align="center", message = FALSE, eval=FALSE}
bins <- seq(0, 22, 1)
...
The distribution of each estimator reveals a trade-off between *variance* and *bias*. We observe that the min-based estimator almost always overestimates. The bulk of the simulated statistics cluster around the $(5, 6]$ bin, but, by design, the estimator can only predict larger numbers. In this case, we say that the min-based estimator is *biased* and has low *variability*. 

In contrast, the mean-based estimator overestimates about as often as it underestimates. This estimator is not *biased* but incurs more *variability* as a result. 

### Median flight delay in `flights`

Let us now return to estimating the median flight delay in the `flights` data frame. Recall that, based on our assumptions, we know the value of the parameter the statistic is trying to estimate. It is the value $-2$. 
...
Regardless of the sampling plan used, sampling strategies demand prudence on the part of the data scientist.  
