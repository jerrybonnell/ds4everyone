...
functions you would expect to use are available by default, such as `abs` and `round`. Other functions that are built into the R language, e.g., for loading in text from a URL or plotting diagrams, are stored in a collection of functions called a *package*. These can be imported by running the function call `library()`, such as `readtext` or `ggplot2`. We used these earlier to conduct our literary analysis in \@ref(literary). We will be making use of another package called [tidyverse](https://www.tidyverse.org/) which is a suite of packages designed for doing data science. We can import it in just one line.    

```{r, warning=FALSE}
library(tidyverse)
```

This line will also tell you which functions from `tidyverse` conflict with functions in base R (or from other packages you have loaded). If this is the first time you are running this, you will get an error message indicating "there is no package called `tidyverse`". You will need to first install it, then run `library()` once again. 
...
We can now apply R to analyze data. We will work with data stored in data frame structures.

Tables are a fundamental way of representing data sets. A table can be viewed in two ways:

* a sequence of named columns that each describe a single attribute of all entries in 
  a data set, or
* a sequence of rows that each contain all information about a single individual in 
  a data set.
...
The table cones has been imported for us; later we will see how, but here we will 
just work with it. First, let's take a look at it.
