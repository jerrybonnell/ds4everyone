...
* **Question 3.3** Which of the following statements, if any, are accurate based on the tibble `with_scaled`? 
  ...
  1. Arnold's scaled Judge Mary score is about 1.9 standard deviations higher than the mean score Arnold received from the four different judges.  
  2. The score Judge Mary gave to Baxter is roughly 0.02 standard deviations below Judge Mary's mean score over all applicants. 
  3. Based on the raw score alone, we can tell the score Arnold received from Judge Mary is higher than the average score Judy Mary gave. 
  4. The standard deviation of Judge Mary's scaled scores is higher than the standard deviation of Judge Olivia's scaled scores. 
...
* **Question 4.6** Write a function called `hist_for_sample` that receives a single argument `sample_size`. This function should accomplish the following: 
  ...
  1. Call repeatedly the function `var_from_sample` with the given `sample_size`, say, 1,000 times. 
  2. Generate a histogram of the simulated sample statistics under density scale. 
  3. Annotate this histogram with (1) a vertical blue line showing where the population parameter is (`pop_var_abs_diff`) and (2) a red point indicating the *mean* of the generated sample statistics. 
