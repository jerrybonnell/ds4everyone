...
You can pull up more information about the dataset by typing the name of it with a single question mark in front of it.
...
We are now ready to provide a definition of tidy data. We defer to [Hadley Wickham](https://en.wikipedia.org/wiki/Hadley_Wickham), a main contributor and maintainer of many of the key packages that compose the `tidyverse`, for a definition. We say that data is tidy when it satisfies three conditions: 

1. Each *variable* forms a column. 
2. Each *observation* forms a row. 
3. Each type of observational unit forms a table. 

Data that exists in any other arrangement is, consequently, *messy*. The goal of this chapter is to learn about methods for *transforming* messy data into tidy data, with some help from R and the tidyverse. 

The `mtcars` dataset above is an example of tidy data, with one small exception we will return to later. Let us look at other examples of datasets that fulfill or violate these properties. 

Suppose that you are keeping track of weekly sales of three different kinds of cookies at a local bakery. By instinct, you decide to keep track of the data in the following table.    
...
```{r}
bakery_tidy <- bakery1 %>%
  pivot_longer(gingerbread:`macadamia nut`, names_to = "type", values_to = "sales")
bakery_tidy
```

Observe how this dataset fulfills the three properties of tidy data; the third property is fulfilled simply because the observational unit we are measuring is the cookie, and we are measuring three variables per observational unit: `week`, `type`, and `sales`. 

When a dataset is expressed in this manner, we say that it is in *long* format because the number of rows is comparatively larger compared to `bakery1` and `bakery2`. Admittedly, this form can make it harder to identify patterns or trends in the data by eye. However, in terms of computational analysis, tidy data makes things much more efficient.  
...
In this section we dive deeper into datasets and learn how to do basic tasks with datasets and query basic information from them.  
...
In its simplest form, a data frame consists of vectors lined up together where each vector has a name. The designation `<dbl>` appearing next to the columns indicates that the column has only double values.
...
Observe that the names of the car models are no longer present. However, we may wish to keep the names of the models as it can bring useful information. `tibble` has thought of a solution to this problem for us: we can add a new column with the row name information. The required function is `rownames_to_column`.  

```{r}
mtcars_tibble <- rownames_to_column(mtcars, var = "model_name")
mtcars_tibble
```

Throughout the course, we will store data using the `tibble` construct. However, because `tibbles` and data frames are close siblings, we may use the terms *tibble* and *data frame* interchangeably when talking about data that is stored in a rectangular format.       
...
If you know the position of a column in the dataset, you can attach a pair of square brackets with a comma and the position to get to the vector. The `cyl` is at position 2 of the data, so we obtain the following. 

```{r}
mtcars_tibble[,2]
```   

Similarly, if we know the position of a row in the dataset, we can use a similar syntax. The following will return all the associated information for the second row of the dataset. 
...
```{r}
pull(class, chemistry_grades) # retrieve all grades in chemistry
min(pull(class, chemistry_grades))  # retrieve the minimum chemistry score
```
...
We can inspect the first few rows of the tibble using the function `head`.
...
The `dplyr` packages provides a suite of functions for providing such transformations. Put another way, `dplyr` provides a *grammar* of data manipulation where each function can be thought of as the *verbs* that act upon the subject, the dataset (in tibble form). In this section we will study the main `dplyr` verbs. 
...
```{r}
mtcars_tibble <- rownames_to_column(mtcars, "model_name")
mtcars_tibble
```

### A fast overview of the verbs

The main important verbs from `dplyr` that we will cover are: 
...
We will also cover the `%>%` operator to coordinate multiple actions seamlessly. 

Be sure to bookmark the `dplyr` [cheatsheet](https://rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) which will come in handy. 
...
The function `mutate` creates a new column using some function of the values of existing columns. Let us see an example before getting into the details.  
...
```{r}
mtcars_with_ratio <- mutate(mtcars_tibble, cyl_gear_ratio = cyl / gear)
mtcars_with_ratio
```
...
Unfortunately, the result is not that interesting. To make something out of this, we need to *summarize* some piece of information using these groups, e.g. the average `mpg` per group as is needed for our task. 
...
### Coordinating multiple actions using the pipe  
...
Observe how the code we have just written is quite cumbersome. It introduces several intermediate products that we do not need, namely, the objects `grouped_by_cl` and `summarized`. It can also be difficult to come up with good names for objects. 

Conveniently, there is a construct called the "pipe" which allows us to pass the results from one function as input to another. The way to use piping is simple.
...
In this section we look to two more transformation techniques that are essential for achieving tidy data. 
...
The third principle of tidy data stated that an observational unit should form a table. However, often times the observational unit we are measuring will be split across multiple tables.  

Let us suppose we are measuring student scores in a classroom. The data is given to us in the form of two tables, one for exams and the other for assignments. We load the scores into our R environment with tibble. 
...
If our observational unit is the *grade*, then it should be clear that the current arrangement of data is not tidy. The data should be kept together in a single table.  
...
To remedy this, we use *pivot* and, in terms of R, the function `pivot_longer`. The syntax for `pivot_longer` can be complex, and so we do not go over it in detail. Here is how we can use it.   
...
The usage above takes `scores_inner`, merges all the assessment columns, creates a new column with name `assessment`, and presents the corresponding values under the column `score`. Observe how we can easily read off the three variables from this table: `name`, `assessment`, and `score`. We can be confident in knowing that this is tidy data.

If we wish to go in the other direction, say for readability, we can use `pivot_wider`. The function `pivot_wider` grabs a pair of columns and spreads the pair into a series of columns. One column of the pair serves as the source for the new column names after spreading. For each value appearing in the source column, the function creates a new column by the name.
...
```{r}
scores_long %>%
  pivot_wider(names_from = assessment, values_from = score, names_prefix = "assess_")
```
...
Let us now return to the discussion of how we can *apply* functions to a column. The meaning of apply is particular. What we mean by this is that we wish to run some function (which can receive an argument and return a value) to each row of a column. This can be especially useful if some column is given in the wrong units or if we wish to cut off the values in a column at some point.  

We can accomplish this using a suite of functions thoughtfully prepared by the package [purrr](https://github.com/rstudio/cheatsheets/blob/master/purrr.pdf). 

```{r, echo=FALSE, fig.align="center", out.width='20%'}
knitr::include_graphics('images/purr_logo.png')
```

The main construct we will be using from `purrr` is something called the *map*. A map applies a function, say the `cutoff` function we wrote earlier, to each element of a vector or list. 
...
Here are some more examples of using map. Let us apply the `passes_100` to an input vector containing a sequence of values from 95 to 105. 

```{r}
map_dbl(95:105, passes_100)
```

Observe how the first five values are less than `100` and, therefore, a run of `100`'s composes the first few values of the returned vector.  
...
By now we have seen enough examples of how to use `map` with a vector. Let us finally learn how to apply `map` to a column of a tibble. 
...
1. Convert the `wt` column from pounds to kilograms.  
2. Cut off the values in `displ` so that no car model has a value larger than `400`.

We can address the first one simply using an anonymous function that multiples each value by the conversion factor for kilograms. Let us test it out first with a simple vector.

```{r}
map_dbl(100:105, function(x) x * 0.454)
```

To incorporate this into a tibble, we encase our `map` inside a call to `mutate`, which modifies the column `wt`. 

```{r}
mtcars_transformed <- mtcars_tibble %>%
  mutate(wt = map_dbl(wt, function(x) x * 0.454))
mtcars_transformed
```

We have successfully applied a function a column in a tibble! Let us turn to the second task. For this, we define a function that computes the minimum between the argument and the value `400`. 
...
We can apply the function to the column `disp` using a similar approach as before.
...
We can inspect visually to see if any values in `disp` are larger than 400 -- they shouldn't be! 
...
```{r}
trouble_temps <- tibble(city = c("Miami", "Boston", "Seattle", "Arlington"),
                        week1 = c(89, 88, 87, NA), 
                        week2 = c(91, NA, 86, 75), 
                        week3 = c(88, 85, 88, NA))
trouble_temps
...
Below, we fill any `NA` in `week1` with 0, in `week2` with 77, and in `week3` with 66.
...
If you want to make an across-the-board replacement of NA with a specific value, you can use `replace(is.na(.), X)`. Here `X` is the value to substitute the missing value with. 
...
Note that if your dataset contains a mixture of strings and numbers, then a straightforward application like this will not work. Instead, you will need to split the process into two steps: first taking care of missing values in the strings columns and then, afterwards, taking care of the missing values in the numeric columns.  


## Case Study: A Course in Computer Science

Before we end this chapter, we present a worked example of a real-life dataset and apply the transformations that are necessary to bring it into tidy data. The goal of this exercise is to demonstrate how all the transformations we have learned in this chapter are not tools meant to be used in isolation, but work best when used together to fulfill a complex goal.  

### Prerequisites

As always, we will need the `tidyverse` to accomplish our task.  

```{r, message = FALSE, warning = FALSE}
library(tidyverse)
```

### The `csc_course` tibble

We will examine a dataset containing scores from a course in Computer Science. 

```{r message = FALSE, warning = FALSE}
csc_course_lab <- read_csv("data/csc_course_lab.csv")
csc_course_hw <- read_csv("data/csc_course_hw.csv")
csc_course_qz <- read_csv("data/csc_course_qz.csv")
csc_course_lab
csc_course_hw
csc_course_qz
```
...
* The dataset is split across three different tables as CSV files: one for lab scores, a second for homework scores, and a third for quiz scores. 
...
We can tell the variables that are being measured in this dataset. 

* *number*, the unique number of each student 
* *assessment*, the kind of assessment, e.g., lab01, hw01, qz01 
* *score*, the received score on the assessment 

It should hopefully be clear that the current format of the dataset violates all three principles of tidy data. Those problems being: 

1. The observational unit is the *student* which is spread across three different tables.
2. There is missing data present which, by definition, is not tidy.
3. We would like to keep the scale of the scores the same for each assessment, i.e., labs, homeworks, and quizzes should all be on a 100 point scale. 
4. Both the student *number* and *score* appears in each column and multiple observations appear in each row. 

Once we have tided the data, we would like to answer the following questions:  

1. What is average lab, homework, and quiz score for each of the students? What are the overall averages for the class? 
2. Which students are performing below the average? 
...
We can accomplish this using the `is.na` function. 
...
We could use `map_dbl` for each of the eleven homeworks and twelve labs, but this can be quite cumbersome to type. The function `across` is equivalent to `map`, but allows us to apply a function to multiple columns at once. Then, as before, we include the incantation inside a call to `mutate`. Here is how we can put everything together. 
...
```{r}
csc_course_merged <- csc_course_merged %>%
  pivot_longer(starts_with("hw") | starts_with("lab") | starts_with("qz"), 
               names_to = "assessment", values_to = "score")
csc_course_merged
```
...
Now let us zoom in on those that are performing less than a 70. 
...
You can imagine how such an analysis could be useful when used mid-semester -- we should check in with those students to see what is going on. 
