...
The package `dplyr` offers five important functions for data manipulation.

* `mutate()`: for adding new attributes from existing attributes
* `select()`: for selecting or deselecting attributes
* `filter()`: for selecting rows
* `summarise()`: for summarizing rows by groups
...
To execute the replacement, we use `str_replace` by matching the pattern `" .*"` (one white space and then any charactrer sequence) with `""`.
...
```{r}
myc2 %>% mutate(which_model = str_replace(make_model, " .*", ""), .before=2) %>% mutate(mpg_disp = mpg * disp, .after=2) -> myc4
```
...
```{r}
myc2 %>%
  mutate(which_model = str_replace(make_model, " .*", ""), .before=2) %>%
