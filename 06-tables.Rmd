...
```{r message = FALSE, warning = FALSE}
csc_course <- read_csv("data/csc_course.csv")
csc_course
```
...
Suppose we want to compute for each index between 1 and 10, the difference of the "hw" value at the index fomr the "lab" value at the previous, that is, `hw10 - lab10`, `hw09 - lab09`, and so.
We make the following plan.

1. Read the data and substitute each NA with 0.
2. Create a data subset consisting of "number" and the ten "lab" scores.
3. Pivot using the "lab" scores, extract the two characters following the prefix "lab", attach them after "number" to create a temporary ID.
4. Repeat the same concerning the homework scores.
5. The temporary ID is common between the two data subsets. Unite the two data sets using the common ID.
6. Using the "lab" and "hw" scores, compute the difference.
7. From the new merged version, remove the temporary ID.
